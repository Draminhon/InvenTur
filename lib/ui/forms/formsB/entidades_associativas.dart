...
final Map<String, dynamic> valoresjson = {
  'tipo_formulario': 'Entidades Associativas e Similares',
};
bool isUpdate = false;

class EntidadesAssociativas extends StatefulWidget {
  final EntidadesAssociativasModel? hospedagemModel;
  final bool? isAdmin;
  const EntidadesAssociativas({super.key, this.hospedagemModel, this.isAdmin});

  @override
  State<EntidadesAssociativas> createState() => _EntidadesAssociativasState();
}

class _EntidadesAssociativasState extends State<EntidadesAssociativas> {
  int currentStep = 0;
    int pesquisadorId = 0;
  ...
  void dispose() {
    ...
    valoresjson.clear();
    valoresjson['tipo_formulario'] = 'Entidades Associativas e Similares';
    isUpdate = false;
    super.dispose();
  }
  ...
  void _enviarFormulario() {
    if (_formKey.currentState?.validate() ?? false) {
      ...
      if (currentStep < pages.length - 1) {
        ...
        _utils.decideSendingOrUpdating(isUpdate, isTheOwner, context, widget.hospedagemModel?.id ?? 0, valoresjson, AppConstants.ENTIDADES_ASSOCIATIVAS);
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      child: Column(children: [
        ...
        RadioFormField(
          title: 'Tipo',
          options: ['Entidades Associativas e Similares'],
          onSaved: (newValue) {
        ...
        CustomTextField(
          name: 'Nome do Dirigente',
          controller: widget.controllers['nomeDoDirigente'],
        ),

        CustomTextField(
          name: 'Cargo',
          controller: widget.controllers['cargo'],
        ),
        CustomTextField(
          name: 'Representação do Setor de',
          controller: widget.controllers['representacaoSetor'],
        ),

        CustomTextField(
          name: 'Número de Associados',
          controller: widget.controllers['numeroDeAssociados'],
          formatter: [FilteringTextInputFormatter.digitsOnly],
          keyboardType: TextInputType.numberWithOptions(),
        ),

        RadioFormField(
          options: ['Municipal', 'Regional', 'Estadual', 'Nacional'],
          title: 'Abrangência',
          onSaved: (newValue) => valoresjson['abrangencia'] = newValue,
          initialValue: isUpdate ? widget.hospedagemModel!.abrangencia : "",
        ),

        RadioFormField(
          options: ['Patronal', 'Trabalhador', 'Setor', 'outro'],
          title: 'Representatividade',
          initialValue:
              isUpdate ? widget.hospedagemModel!.representividade : "",
          onSaved: (newValue) => valoresjson['representividade'] = newValue,
        ),
        CustomTextField(
          name: 'CNPJ',
        ...
        textLabel(name: 'Cadastros, Classificações, Filiações e  Outros', fontWeight: FontWeight.bold,),
        SizedBox(height: 55.h,), 
        textLabel(name: 'Fórum', ),
        CustomTextField(name: 'Categoria/Tipo/Classificação/Número', controller: widget.controllers['forum'],),
        textLabel(name: 'Conselho', ),
        CustomTextField(name: 'Categoria/Tipo/Classificação/Número', controller: widget.controllers['conselho'],),
        textLabel(name: 'Federação', ),
        CustomTextField(name: 'Categoria/Tipo/Classificação/Número', controller: widget.controllers['federacao'],),
        textLabel(name: 'Associação', ),
        CustomTextField(name: 'Categoria/Tipo/Classificação/Número', controller: widget.controllers['associacao'],),
        textLabel(name: 'Outros', ),
        CustomTextField(name: 'Categoria/Tipo/Classificação/Número', controller: widget.controllers['outrosEntidade'],),
        SizedBox(
          height: 100.w,
        ),
      ]),
...
class _FuncionamentoState extends State<Funcionamento>
  ...
  Widget build(BuildContext context) {
    ...
    return SingleChildScrollView(
      ...
      CheckboxGroupFormField(
        title: 'Serviços e Atividades',
        initialValue:
            isUpdate == true ? widget.hospedagemModel!.outrasIntalacoes : [],
        onSaved: (p0) => valoresjson['outrasIntalacoes'] = p0,
        options: [
'Assessoria técnica', 'Assessoria jurídica', 'Oferta de cursos de capacitação', 'Realização de eventos setoriais', 'Realização de eventos aberto ao público', 'Promoção/divulgação do setor', 'Cadastro de oferta e demanda de pessoal', 'Intercâmbio de experiências', 'Central de compras', 'Captação e realização de congressos e eventos', 'Edição de boletins/revistas/jornais', 'outro'
        ],
      ),
